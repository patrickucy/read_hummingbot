...
class SimpleXEMMConfig(BaseClientModel):
    ...
    rebalance_threshold: Decimal = Field(0.05, client_data=ClientFieldData(
        prompt_on_new=True, prompt=lambda mi: "Inventory ratio threshold to trigger a rebalance"))
    rebalance_percentage: Decimal = Field(0.2, client_data=ClientFieldData(
        prompt_on_new=True, prompt=lambda mi: "Percentage of inventory ratio to buy when rebalancing"))
...
class SimpleXEMM(ScriptStrategyBase):
    ...
    def on_tick(self):
        ...
        # rebalance if inventory ratio is below threshold
        inventory_ratio = self.get_inventory_ratio(self.config.maker_exchange, self.config.maker_pair)
        if inventory_ratio <= self.config.rebalance_threshold:
            self.rebalance("buy")
        elif 1 - inventory_ratio <= self.config.rebalance_threshold:
            self.rebalance("sell")

        taker_buy_result = self.connectors[self.config.taker_exchange].get_price_for_volume(self.config.taker_pair,
                                                                                            True,
                                                                                            self.config.order_amount)
        taker_sell_result = self.connectors[self.config.taker_exchange].get_price_for_volume(self.config.taker_pair,
                                                                                             False,
                                                                                             self.config.order_amount)
        ...
        if not self.buy_order_placed:
            ...
            buy_order_adjusted = self.connectors[self.config.maker_exchange].budget_checker.adjust_candidate(buy_order,
                                                                                                             all_or_none=False)
        ...
        for maker_order in self.get_active_orders(connector_name=self.config.maker_exchange):
            cancel_timestamp = maker_order.creation_timestamp / 1000000 + self.config.max_order_age
            if maker_order.is_buy:
                buy_cancel_threshold = taker_sell_result.result_price * Decimal(1 - self.config.min_spread_bps / 10000)
                if maker_order.price > buy_cancel_threshold or cancel_timestamp < self.current_timestamp:
                    self.logger().info(f"Cancelling buy order: {maker_order.client_order_id}")
                    self.cancel(self.config.maker_exchange, maker_order.trading_pair, maker_order.client_order_id)
                ...
                if maker_order.price < sell_cancel_threshold or cancel_timestamp < self.current_timestamp:
                    self.logger().info(f"Cancelling sell order: {maker_order.client_order_id}")
                    self.cancel(self.config.maker_exchange, maker_order.trading_pair, maker_order.client_order_id)
                    self.sell_order_placed = False
            self.current_timestamp += self.config.max_order_age
        return

    def buy_hedging_budget(self) -> Decimal:
        balance = self.connectors[self.config.taker_exchange].get_available_balance("ETH")
        # buying all available balance!
        return balance
    ...
    def did_fill_order(self, event: OrderFilledEvent):
        # once maker order filled ! Then perform arbitrage
        if event.trade_type == TradeType.BUY and self.is_active_maker_order(event):
            ...
            self.buy_order_placed = False  # mark this order as filled, and arbitrage is done
    ...
    def place_buy_order(self, exchange: str, trading_pair: str, amount: Decimal,
        ...
        buy_order = OrderCandidate(trading_pair=trading_pair, is_maker=False, order_type=order_type,
                                   order_side=TradeType.BUY, amount=amount, price=buy_price_with_slippage)
    ...
    def place_sell_order(self, exchange: str, trading_pair: str, amount: Decimal,
        ...
        sell_order = OrderCandidate(trading_pair=trading_pair, is_maker=False, order_type=order_type,
                                    order_side=TradeType.SELL, amount=amount, price=sell_price_with_slippage)
    ...
    def exchanges_df(self) -> pd.DataFrame:
        ...
        maker_buy_result = self.connectors[self.config.maker_exchange].get_price_for_volume(self.config.maker_pair,
                                                                                            True,
                                                                                            self.config.order_amount)
        maker_sell_result = self.connectors[self.config.maker_exchange].get_price_for_volume(self.config.maker_pair,
                                                                                             False,
                                                                                             self.config.order_amount)
        taker_buy_result = self.connectors[self.config.taker_exchange].get_price_for_volume(self.config.taker_pair,
                                                                                            True,
    ...
    def get_inventory_ratio(self, exchange: str, trading_pair: str) -> float:
        """
        Returns the ratio of the base asset to the quote asset for the specified exchange and trading pair
        """
        base, quote = trading_pair.split('-')
        mid_price = self.connectors[exchange].get_mid_price(trading_pair)
        base_bal = self.connectors[exchange].get_balance(base)
        base_bal_in_quote = base_bal * mid_price
        quote_bal = self.connectors[exchange].get_balance(quote)
        ratio = float(base_bal_in_quote / (base_bal_in_quote + quote_bal))
        self.logger().info(f"Inventory ratio on {exchange} for {trading_pair}: {ratio:.4f}")
        return ratio

    def get_base_amount_for_rebalance_percentage(self, exchange: str, trading_pair: str, percentage: float) -> float:
        """
        Returns the amount of base asset to buy/sell to rebalance inventory ratio
        """
        base, quote = trading_pair.split('-')
        mid_price = self.connectors[exchange].get_mid_price(trading_pair)
        base_bal = self.connectors[exchange].get_balance(base)
        base_bal_in_quote = base_bal * mid_price
        quote_bal = self.connectors[exchange].get_balance(quote)
        base_rebalance_amount = percentage * (base_bal_in_quote + quote_bal) / mid_price
        self.logger().info(
            f"{percentage * 100:.2f}% inventory ratio on {exchange} == {base_rebalance_amount:2f} {base}")
        return base_rebalance_amount

    def rebalance(self, side: str):
        """
        Places orders on both exchanges to rebalance inventory ratio
        """
        rebalance_amount = self.get_base_amount_for_rebalance_percentage(self.config.maker_exchange,
                                                                         self.config.maker_pair,
                                                                         self.config.rebalance_percentage)
        if side == "buy":
            self.logger().info(
                f"Rebalance: Buying {rebalance_amount} on {self.config.maker_exchange} and selling {rebalance_amount} on {self.config.taker_exchange}")
            # Maker buy
            self.place_buy_order(self.config.maker_exchange, self.config.maker_pair, rebalance_amount, OrderType.MARKET)
            self.place_sell_order(self.config.taker_exchange, self.config.taker_pair, rebalance_amount,
                                  OrderType.MARKET)
        else:
            self.logger().info(
                f"Rebalance: Selling {rebalance_amount} on {self.config.maker_exchange} and buying {rebalance_amount} on {self.config.taker_exchange}")
            # Maker sell
            self.place_sell_order(self.config.maker_exchange, self.config.maker_pair, rebalance_amount,
                                  OrderType.MARKET)
            self.place_buy_order(self.config.taker_exchange, self.config.taker_pair, rebalance_amount, OrderType.MARKET)
        return
